...
// In this example, there is the challenge to add feature to
// Print the output of the philosophers from start to finish the
// dinning steps in the correct order.
...
var (
	hunger    = 3
	eatTime   = (time.Millisecond * time.Duration(rand.Intn(1000)))
...
var orderFinished []string // the order in which the Philosophers finish dining; part of the challenge
...
func main() {
	...
	// *** challenge solution - adding sleep time
	...
	// Order finished
	fmt.Println("Order fifinished:")
	for i, philosopher := range orderFinished {
		fmt.Printf("#%d: %s\n", i, philosopher)
	}
}
